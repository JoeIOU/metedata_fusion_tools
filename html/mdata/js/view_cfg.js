...
var entity_info_list = url_root + "/services/queryViewList";
...
var entitySetup_url = "ui_entity_table.html?readOnly=0";
...
var gl_data_views_id=null;
var gl_view_inputs_id=null;
var gl_view_outputs_id=null;
...
axios.get(url_entity.format("data_views"))
	.then(res => {
		var data = null;
		var d = res.data.data;
		if (d) {
			data = d[0];
			var entity_id = data['md_entity_id'];
			gl_data_views_id = entity_id;
		}
	});
axios.get(url_entity.format("view_inputs"))
	.then(res => {
		var data = null;
		var d = res.data.data;
		if (d) {
			data = d[0];
			var entity_id = data['md_entity_id'];
			gl_view_inputs_id = entity_id;
		}
	});
axios.get(url_entity.format("view_outputs"))
	.then(res => {
		var data = null;
		var d = res.data.data;
		if (d) {
	...
	var app = new Vue({
		...
		methods: {
			handleClick(data) {
				...
				url=generate_url(null,gl_data_views_id,'',GL_ENTITY_ID,true,'0')
				app.iframe_url = url;

				parent_entity_id=gl_data_views_id
				entity_id=gl_view_inputs_id
				_row_id_=GL_ENTITY_ID
				url=generate_url(parent_entity_id,entity_id,'',_row_id_,true,'0')
				app.iframe_url2 = url;
				entity_id=gl_view_outputs_id
				_row_id_=GL_ENTITY_ID
				url=generate_url(parent_entity_id,entity_id,'',_row_id_,true,'0')
				app.iframe_url3 = url;
			},
